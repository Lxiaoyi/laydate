/**
 ...
 经过修改，插件format为YYYY-MM报错问题处理(这里只处理了YYYY-MM格式，如果有其它格式如YYYY/MM请于293和500行进行修改)
 增加isMonth属性，可以只选择年月
 */
    ...
    Dates.check = function(){
        ...
        if(value.replace(/\s/g, '') !== ''){
            if(!exp.test(value)){
                ...
                //自己修改添加，处理年月报错问题
                if(Dates.options.format == "YYYY-MM"){
                    arr.push("01");
                }
                if(arr[1] < 1){
                    arr[1] = 1;
...
//初始化面板数据
    Dates.initDate = function(format){
        ...
        //自己修改添加，处理format为年月时，年、月无法选择问题
        if(format == "YYYY-MM"){
            elemIndexMap = Dates.getEachElementIndex("YYYY-MM-DD")
        }
